...
Description:	A hash table template that uses linear probing.
...
public:
	...
	// getNumElements() will return the number of elements in the table
	int getNumElements(){return currentSize; };

	// getTableSize() will return the size of the table
...
private:
    ...
    /*  Modification: findPos() will now increment the mutable private data
        ...
        Modification: Probing is now done linearly.
    */
    int findPos(const HashedObj & x) const
	{
		...
		{
			++collisions;
			currentPos += 1;  // Compute ith linear probe
			if (currentPos >= array.size())
				currentPos -= array.size();
		}
